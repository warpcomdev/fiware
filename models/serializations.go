...
func (x Manifest) Serialize(s serialize.Serializer) {
	...
	if len(x.Subscriptions) > 0 {
		...
		for _, k := range slices.Sorted(maps.Keys(x.Subscriptions)) {
			v := x.Subscriptions[k]
			s.BeginBlock(k)
			v.Serialize(s)
			s.EndBlock()
	...
	if len(x.Rules) > 0 {
		s.BeginBlock("rules")
		for _, k := range slices.Sorted(maps.Keys(x.Rules)) {
			v := x.Rules[k]
			s.BeginBlock(k)
			v.Serialize(s)
			s.EndBlock()
	...
	if len(x.Verticals) > 0 {
		...
		for _, k := range slices.Sorted(maps.Keys(x.Verticals)) {
			v := x.Verticals[k]
			s.BeginBlock(k)
			v.Serialize(s)
			s.EndBlock()
	...
	if len(x.Panels) > 0 {
		...
		for _, k := range slices.Sorted(maps.Keys(x.Panels)) {
			v := x.Panels[k]
			s.BeginBlock(k)
			v.Serialize(s)
			s.EndBlock()
...
func (x Attribute) Serialize(s serialize.Serializer) {
	...
	if len(x.LongtermOptions) > 0 {
		...
		sort.Strings(x.LongtermOptions)
...
func (x Entity) Serialize(s serialize.Serializer) {
	...
	s.KeyString("entityType", string(x.Type))
	s.BeginBlock("attrs")
	for _, k := range slices.Sorted(maps.Keys(x.Attrs)) {
		v := x.Attrs[k]
		s.KeyRaw(k, v, true)
	}
	s.EndBlock()
	if len(x.MetaDatas) > 0 {
		...
		for _, k := range slices.Sorted(maps.Keys(x.MetaDatas)) {
			v := x.MetaDatas[k]
			s.KeyRaw(k, v, true)
		}
		s.EndBlock()
...
func (x Environment) Serialize(s serialize.Serializer) {
	...
	for _, k := range slices.Sorted(maps.Keys(x.NotificationEndpoints)) {
...
func (x DeploymentManifest) Serialize(s serialize.Serializer) {
	if len(x.Sources) > 0 {
		...
		for _, k := range slices.Sorted(maps.Keys(x.Sources)) {
			v := x.Sources[k]
			s.BeginBlock(k)
			v.Serialize(s)
			s.EndBlock()
...
func (x NotificationCustom) Serialize(s serialize.Serializer) {
	...
	if len(x.Qs) > 0 {
		s.BeginBlock("qs")
		for _, k := range slices.Sorted(maps.Keys(x.Qs)) {
			v := x.Qs[k]
			s.KeyString(k, string(v))
		}
		s.EndBlock()
...
func (x User) Serialize(s serialize.Serializer) {
	...
	if len(x.Options) > 0 {
		...
		for _, k := range slices.Sorted(maps.Keys(x.Options)) {
			v := x.Options[k]
			s.KeyRaw(k, v, false)
		}
		s.EndBlock()
